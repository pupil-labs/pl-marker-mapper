...
def setup(recording_dir):
    ...
    return camera, marker_detector, frames


def visualize_results(camera, frame, markers, surface, localization):
    orig_img = frame.bgr
    ...
    if localization is not None:
        ...
        orig_img = helpers.draw_markers(orig_img, marker_ids, vertices_dist, surface)
        undist_img = helpers.draw_markers(
            undist_img, marker_ids, vertices_undist, surface
        )

        surface_boundary_undist = utils.get_surface_boundary(surface2image)
        helpers.draw_surface(undist_img, surface_boundary_undist)

        surface_boundary_dist = utils.get_surface_boundary(
            surface2image, distorted=True, camera=camera
        )
        helpers.draw_surface(orig_img, surface_boundary_dist)
